...
export default function RegistrationForm() {
  ...
  const [coCourses1st, setCoCourses1st] = createStore([]);
  const [eduCourses1st, setEduCourses1st] = createStore([]);
  const [gseCourses1st, setGseCourses1st] = createStore([]);
  const [firstMajorCourses1st, setFirstMajorCourses1st] = createStore([]);
  const [secondMajorCourses1st, setSecondMajorCourses1st] = createStore([]);
  const [coCourses2nd, setCoCourses2nd] = createStore([]);
  const [eduCourses2nd, setEduCourses2nd] = createStore([]);
  const [gseCourses2nd, setGseCourses2nd] = createStore([]);
  const [firstMajorCourses2nd, setFirstMajorCourses2nd] = createStore([]);
  const [secondMajorCourses2nd, setSecondMajorCourses2nd] = createStore([]);
  ...
  const getCourseDetails = async (arr, which) => {
    ...
    if (which === "dropped") {
      ...
      setDroppedTotalCU(total_cu);
      setDroppedTotalProgFee(total_amt);
    }

    if (which === "added") {
      setDetAddedCourses(c);
      setAddedTotalCU(total_cu);
      setAddedTotalProgFee(total_amt);
    }
  };
  ...
  const addThisCourse = async (courseCode) => {
    ...
    if (pickedCourses) {
      ...
      for (let index = 0; index < coursesArray.length; index++) {
        var picked = checkIfAdded(courseCode);
  ...
  const showDropThisCoursePop = async (courseCode, courseCu, courseAmt) => {
    setDropCourseCode(courseCode);
    setDropCourseCU(courseCu);
    setDropCourseAmt(courseAmt);
    setShowDropConfirmation(true);
  };

  const dropThisCourse = async (courseCode) => {
    var new_courses = [];

    if (droppedCourses) {
      var coursesArray = Object.keys(droppedCourses).map(
        (key) => droppedCourses[key]
      );
      for (let index = 0; index < coursesArray.length; index++) {
        var dropped = checkIfDropped(courseCode);
    ...
    setDroppedTotalCU(new_cu);
    setDroppedTotalProgFee(new_amt);
    ...
    Promise.all([request1, request2])
      .then(([data1, data2]) => {
        ...
        var registration = fetchRegistration();
        if (registration.dropped_courses) {
          setDroppedCourses(JSON.parse(registration.dropped_courses));
        }
      })
  ...
  const completeAddDrop = async () => {
    setIsProcessing(true);
    var new_bal = parseInt(portalWallet()) - parseInt(addedTotalProgFee());
    ...
    var formNumber = "A-D" + sem_rep + params.customId + year;
    try {
      const request1 = fetch(
        VITE_API_URL + "/api/edit-portal-wallet/" + params.customId,
        {
        ...
        <Show
          ...
          fallback={
            <>
              ...
              <Show when={showDropConfirmation()}>
                <div class="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-90 h-screen w-screen flex items-center">
                  <div class="w-80 sm:w-10/12 lg:w-6/12 mx-auto bg-white rounded-md p-6">
                    <h2 class="text-center text-blue-900 font-semibold">
                      Please Confirm
                  ...
                  <div class="bg-yellow-100 rounded-md border border-yellow-200 p-1 space-y-0.5">
                    <Switch>
                      ...
                      >
                        ...
                        <br />
                        You have began your add/drop. Please ensure to pick your
                                ...
                                <tr>
                                  <td class="p-3 text-sm">
                                    <span class="bg-yellow-100 rounded-md border border-yellow-200 p-1 space-y-0.5 block">
                                      <b>
                                        Have you added/dropped all your
                                        appropriate courses for the semester?
                                      </b>
                                      <br />
                                      <Show
                                ...
                                <tr class="border-b border-black">
                                  <td class="p-4">
                                    <span class="flex justify-between">
                                      ...
                                      <span class="border-x border-black">
                                        Total Charges:{" "}
                                        <b>
                                          {formatter.format(
                                            parseInt(addedTotalProgFee())
                              ...
                              <tbody>
                                <tr>
                                  <td>
                                    <Show
                                      ...
                                      fallback={
                                        <span class="block bg-yellow-100 rounded-md border border-yellow-200  p-1 space-y-0.5">
                                          You can't complete (submit and print)
                                          your add/drop because your portal
                                          wallet is insufficient. Please proceed
                                          to make payment and return to this
                                    ...
                                    >
                                      <span class="block bg-yellow-100 rounded-md border border-yellow-200  p-1 space-y-0.5">
                                        Click on the button below to complete
                                        (submit and print) your registration.
                                        <br />
                                        <Show
                                          when={isProcessing()}
                                          fallback={
                                            <button
                                              onClick={() => completeAddDrop()}
                                              class="green-btn p-3 text-center hover:opacity-60"
                                            >
                                              Submit & Print My Add/Drop
                                            </button>
                                          }
                                        >
                                          <button
